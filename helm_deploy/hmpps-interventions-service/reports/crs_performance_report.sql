COPY (
    WITH attended_sessions AS (
        select count(app.id) AS attended, min(app.appointment_time) as first_appointment, aps.referral_id
        from appointment app
                 join delivery_session_appointment apsa on app.id = apsa.appointment_id
                 join delivery_session aps on apsa.delivery_session_id = aps.id
        where attended in ('YES', 'LATE')
        group by aps.referral_id
    ),
         attempted_sessions AS (
             select count(app.id) AS attempted, aps.referral_id
             from appointment app
                      join delivery_session_appointment apsa on app.id = apsa.appointment_id
                      join delivery_session aps on apsa.delivery_session_id = aps.id
    ...
    FROM
        referral r
            ...
            LEFT JOIN attended_sessions shows ON (shows.referral_id = r.id) --❗️should be linked to referrals instead, sessions are static
            LEFT JOIN attempted_sessions atts ON (atts.referral_id = r.id) --❗️should be linked to referrals instead, sessions are static
