...
export default class EditOasysRiskInformationForm {
  ...
  /* async data(): Promise<FormData<Partial<SentOasysRiskInformation>>> {
      const result = await FormUtils.runValidations({
        request: this.request,
        validations: EditOasysRiskInformationForm.validations(),
      })
      ...
      return {
        paramsForUpdate: null,// change { additionalRiskInformation: this.request.body['additional-risk-information'] },
        error: null,
      }
    } */
  ...
  get editedSentRiskInformation(): SentOasysRiskInformation {
    return {
