...
OptionSelect.prototype.init = function () {
  ...
  this.$optionSelect.addEventListener(
    'change',
    function (event) {
...
OptionSelect.prototype.attachCheckedCounter = function attachCheckedCounter(checkedString) {
  this.$optionSelect
    ...
    .insertAdjacentHTML(
      'afterend',
      '<div class="dm-option-select__selected-counter js-selected-counter">' + checkedString + '</div>'
    )
}
...
OptionSelect.prototype.setupHeight = function setupHeight() {
  ...
  this.setContainerHeight(
    position + parseFloat(window.getComputedStyle(lastVisibleCheckbox, null).height.replace('px', '')) / 1.5
  )
}

$(() => {
  new OptionSelect(document.getElementById('option-select')).init()
})
