...
@JsonTest
class SentReferralWithActionPlanDTOTest(@Autowired private val json: JacksonTester<SentReferralWithActionPlanIdDTO>) {
  @Test
  fun `sent referral requires reference number, sent timestamp, sentBy`() {
    ...
    assertThrows<RuntimeException> {
      SentReferralWithActionPlanIdDTO.from(referral)
    }
    ...
    assertDoesNotThrow { SentReferralWithActionPlanIdDTO.from(referral) }
  }
  ...
  fun `sent referral includes all draft fields and action plan id`() {
    ...
    val out = json.write(SentReferralWithActionPlanIdDTO.from(referral))
  ...
  @Test
  fun `sent referral includes all draft fields and action plan id is null when no plan exists`() {
    val id = UUID.fromString("3B9ED289-8412-41A9-8291-45E33E60276C")
    val createdAt = OffsetDateTime.parse("2020-12-04T10:42:43+00:00")
    val sentAt = OffsetDateTime.parse("2021-01-13T21:57:13+00:00")
    val sentBy = AuthUser("id", "source", "username")
