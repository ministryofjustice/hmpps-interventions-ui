...
internal class ActionPlanServiceTest {
  ...
  fun `successful update action plan`() {
    ...
    val draftActionPlanUpdate = DraftActionPlanDTO.from(SampleData.sampleActionPlan(id = draftActionPlanId, numberOfSessions = 5))
    ...
    val updatedDraftActionPlanResponse = actionPlanService.updateActionPlan(draftActionPlan, draftActionPlanUpdate)
  ...
  fun `update action plan fails validation - number of sessions negative`() {
    val draftActionPlanId = UUID.randomUUID()
    val draftActionPlan = SampleData.sampleActionPlan(id = draftActionPlanId)
    val draftActionPlanUpdate = DraftActionPlanDTO.from(SampleData.sampleActionPlan(id = draftActionPlanId, numberOfSessions = -1))

    val exception = Assertions.assertThrows(ValidationError::class.java) {
      actionPlanService.updateActionPlan(draftActionPlan, draftActionPlanUpdate)
    }
  ...
  fun `update action plan fails validation - number of sessions zero`() {
    val draftActionPlanId = UUID.randomUUID()
    val draftActionPlan = SampleData.sampleActionPlan(id = draftActionPlanId)
    val draftActionPlanUpdate = DraftActionPlanDTO.from(SampleData.sampleActionPlan(id = draftActionPlanId, numberOfSessions = 0))
  ...
  fun `update action plan fails validation - number of sessions is missing`() {
    val draftActionPlanId = UUID.randomUUID()
    val draftActionPlan = SampleData.sampleActionPlan(id = draftActionPlanId)
    val draftActionPlanUpdate = DraftActionPlanDTO.from(SampleData.sampleActionPlan(id = draftActionPlanId, numberOfSessions = null))
